...
public class NoPermaMP {
    ...
    public double motion_profile(double max_acceleration, double max_velocity, double distance,double current_dt) {
        ...
        double acceleration_distance = 0.5 * max_acceleration * Math.pow(current_dt, 2);

        if (acceleration_distance > halfway_distance) {
            acceleration_dt = Math.sqrt(halfway_distance / (0.5 * max_acceleration));
        }

        acceleration_distance = 0.5 * max_acceleration * Math.pow(current_dt, 2);
        ...
        if (current_dt < acceleration_dt) {
            ...
            return acceleration_distance + cruise_distance + max_velocity * deacceleration_time - 0.5 * max_acceleration * Math.pow(current_dt,2);
        }
    }
}
